...
\begin{itemize}
    \item \textbf{Train-clean:} 80\% of images from \texttt{train/good}, used exclusively to build the nominal representation.
...
All splits are generated reproducibly using a fixed random seed (42), with split indices stored in JSON format to ensure consistency across experiments. Table~\ref{tab:split_statistics} summarizes the dataset composition for each category and split, and Figure~\ref{fig:split_distribution} provides a visual representation of the distribution.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{../outputs/visualizations/02_data_preparation/split_distribution.png}
    \caption{Visual representation of the data split distribution across the three MVTec~AD categories, showing the number of normal and anomalous samples in each split.}
...
Since both PatchCore and PaDiM are inference-only or analytical methods that bypass gradient-based optimization on the target domain, traditional training loss curves are not applicable. Instead, we evaluate model performance using threshold-independent metrics, specifically AUROC and AUPRC, to characterize the trade-off between true positive and false positive rates across all possible operating points.
...
\begin{enumerate}
    \item Compute image-level anomaly scores for all validation samples.
...
We train three PatchCore variants with coreset ratios $\rho \in \{1\%, 5\%, 10\%\}$ on the shifted domain (Train-shift), calibrate thresholds on Val-shift, and evaluate on Test-shift. This configuration isolates the effect of coreset size by keeping the domain fixed (shifted), avoiding other influences.
